...
public class VaccinationAdminCodeIsValid extends ValidationRule<DqaVaccination> {
  ...
  public VaccinationAdminCodeIsValid() {
    ruleDetections.addAll(Arrays
        .asList(Detection.VaccinationAdminCodeIsNotSpecific,
  ...
  protected ValidationRuleResult executeRule(DqaVaccination target, DqaMessageReceived m) {
    ...
    if (vaccineCvxCode == null) {
      vaccineCvxCode =
          this.repo.getFirstRelatedCodeForCodeIn(CodesetType.VACCINATION_NDC_CODE, ndcCode,
    ...
    if (vaccineCvxCode != null) {
      ...
      if (CvxConceptType.UNSPECIFIED == concept) {
        ...
        issues
            .add(Detection.VaccinationAdminCodeIsValuedAsNotAdministered.build((cvxCode), target));
