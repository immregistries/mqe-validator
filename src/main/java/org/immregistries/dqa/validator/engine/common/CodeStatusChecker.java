package org.immregistries.dqa.validator.engine.common;

/**
 * The point of this class is to check all of the coded values against the code tables. this should
 * represent all of the codes in the system that we are getting the status for.
 *
 * @author Josh
 */
public class CodeStatusChecker {

  // handleCodeReceived(nextOfKin.getRelationship(),
  // PotentialIssues.Field.NEXT_OF_KIN_RELATIONSHIP);
  // handleCodeReceived(vaccination.getAction(), PotentialIssues.Field.VACCINATION_ACTION_CODE);
  // handleCodeReceived(vaccination.getCompletion(),
  // PotentialIssues.Field.VACCINATION_COMPLETION_STATUS);
  // handleCodeReceived(vaccination.getInformationSource(),
  // PotentialIssues.Field.VACCINATION_INFORMATION_SOURCE);
  // handleCodeReceived(vaccination.getAdminCpt(), PotentialIssues.Field.VACCINATION_CPT_CODE);
  // handleCodeReceived(vaccination.getAdminCvx(), PotentialIssues.Field.VACCINATION_CVX_CODE);
  // handleCodeReceived(vaccination.getManufacturer(),
  // PotentialIssues.Field.VACCINATION_MANUFACTURER_CODE, vaccination.isAdministered());
  // handleCodeReceived(vaccination.getProduct(), PotentialIssues.Field.VACCINATION_PRODUCT);
  // handleCodeReceived(vaccination.getAmountUnit(),
  // PotentialIssues.Field.VACCINATION_ADMINISTERED_UNIT, vaccination.isAdministered() &&
  // amountValued);
  // handleCodeReceived(vaccination.getBodyRoute(), PotentialIssues.Field.VACCINATION_BODY_ROUTE,
  // vaccination.isAdministered());
  // handleCodeReceived(vaccination.getBodySite(), PotentialIssues.Field.VACCINATION_BODY_SITE,
  // vaccination.isAdministered());
  // handleCodeReceived(vaccination.getConfidentiality(),
  // PotentialIssues.Field.VACCINATION_CONFIDENTIALITY_CODE);
  // handleCodeReceived(vaccination.getOrderedBy(), PotentialIssues.Field.VACCINATION_ORDERED_BY,
  // vaccination.isAdministered());
  // handleCodeReceived(vaccination.getEnteredBy(), PotentialIssues.Field.VACCINATION_RECORDED_BY);
  // handleCodeReceived(vaccination.getRefusal(), PotentialIssues.Field.VACCINATION_REFUSAL_REASON,
  // vaccination.isCompletionRefused());
  // handleCodeReceived(observation.getValueType(), PotentialIssues.Field.OBSERVATION_VALUE_TYPE);
  // handleCodeReceived(observation.getObservationIdentifier(),
  // PotentialIssues.Field.OBSERVATION_IDENTIFIER_CODE);
  // handleCodeReceived(patientImmunity.getImmunity(), PotentialIssues.Field.PATIENT_IMMUNITY_CODE);
  // // handleCodeReceived(vaccinationVIS.getDocument(),
  // handleCodeReceived(vaccinationVIS.getCvx(), PotentialIssues.Field.VACCINATION_VIS_CVX_CODE,
  // vaccination.isAdministered());
  // handleCodeReceived(vaccination.getFinancialEligibility(),
  // PotentialIssues.Field.VACCINATION_FINANCIAL_ELIGIBILITY_CODE,
  // handleCodeReceived(header.getAckTypeApplication(), PotentialIssues.Field.HL7_MSH_APP_ACK_TYPE);
  // handleCodeReceived(header.getAckTypeAccept(), PotentialIssues.Field.HL7_MSH_ACCEPT_ACK_TYPE);
  // handleCodeReceived(header.getProcessingStatus(), PotentialIssues.Field.HL7_MSH_PROCESSING_ID);
  // handleCodeReceived(header.getCountry(), PotentialIssues.Field.HL7_MSH_COUNTRY_CODE);
  // handleCodeReceived(header.getCharacterSet(), PotentialIssues.Field.HL7_MSH_CHARACTER_SET);
  // handleCodeReceived(header.getCharacterSetAlt(),
  // PotentialIssues.Field.HL7_MSH_ALT_CHARACTER_SET);
  // handleCodeReceived(patient.getBirthOrder(), PotentialIssues.Field.PATIENT_BIRTH_ORDER);
  // handleCodeReceived(patient.getEthnicity(), PotentialIssues.Field.PATIENT_ETHNICITY);
  // handleCodeReceived(patient.getSex(), PotentialIssues.Field.PATIENT_GENDER);
  // handleCodeReceived(patient.getName().getType(), PotentialIssues.Field.PATIENT_NAME_TYPE_CODE);
  // handleCodeReceived(patient.getFacility().getId(),
  // PotentialIssues.Field.PATIENT_PRIMARY_FACILITY_ID);
  // handleCodeReceived(patient.getPrimaryLanguage(),
  // PotentialIssues.Field.PATIENT_PRIMARY_LANGUAGE);
  // handleCodeReceived(patient.getPhysician(), PotentialIssues.Field.PATIENT_PRIMARY_PHYSICIAN_ID);
  // handleCodeReceived(patient.getProtection(),
  // PotentialIssues.Field.PATIENT_PROTECTION_INDICATOR);
  // handleCodeReceived(patient.getPublicity(), PotentialIssues.Field.PATIENT_PUBLICITY_CODE);
  // handleCodeReceived(patient.getRace(), PotentialIssues.Field.PATIENT_RACE);
  // handleCodeReceived(patient.getFinancialEligibility(),
  // PotentialIssues.Field.PATIENT_VFC_STATUS);
  // handleCodeReceived(phone.getTelUse(), piPhoneTelUse);
  // handleCodeReceived(phone.getTelEquip(), piPhoneTelEquip);
  // handleCodeReceived(address.getCountry(), piAddressCountry);
  // handleCodeReceived(address.getState(), piAddressState, countryCodeReceived);
  // handleCodeReceived(address.getCountyParish(), piAddressCounty,
  // address.getState().getCodeReceived());
}
