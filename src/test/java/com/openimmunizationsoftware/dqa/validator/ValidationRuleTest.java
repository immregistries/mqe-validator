...
public class ValidationRuleTest {
	...
	public void whatClassDoesItReport() {
		ValidationRule<Vaccination> vr = new VaccinationAdminAfterBirthDate();
...
//  for a unit-test I want to test each rule, which can usually be done by calling the methods in the implementations. 
	...
	public void test3() {
		...
		ValidationRule<Patient> vr = new PatientBirthDateIsValid();
		List<IssueFound> issues = vr.executeRule(p, mr).getIssues();
	...
	public void patientRules() {
		...
		MessageReceived mr = getFreshMessage();
		
		mr.getPatient().setBirthDateString("2016-01-01");
		
		for (ValidationRule rule : patientRules) {
			try {
				List<IssueFound> ruleIissues  = rule.executeRule(mr.getPatient(), mr).getIssues();
		...
		assertEquals("should be some issues", 11, issues.size());
	}
	...
	public void vaccinationRules() throws Exception {
		...
		for (ValidationRule<Vaccination> vr : vv) {
			for (Vaccination v : mr.getVaccinations()) {
				List<IssueFound> issues = vr.executeRule(v, mr).getIssues();
			}
		}
	}
	...
	private MessageReceived getFreshMessage() {
		...
		a.setCountryCode("US");
		p.setBirthMultiple("N");
		p.setBirthOrderCode("1");
//		p.setBirthDateString("2016-01-01");
		mr.setPatient(p);
		return mr;
	}
