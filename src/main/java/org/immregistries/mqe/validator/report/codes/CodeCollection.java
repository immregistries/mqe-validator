...
public class CodeCollection {
  ...
  public CodeCollection(MqeMessageReceived message) {
    this.codeCountList = collectMessageCodesNew(message);
  }

  private List<CollectionBucket> collectMessageCodesNew(MqeMessageReceived message) {
    ...
    List<MqeVaccination> vaccineList = message.getVaccinations();
    // merge vaccine list with message header list
    MqeMessageHeader mqeMessageHeader = message.getMessageHeader();
    // merge vaccine and message header list with next of kin list
    List<MqeNextOfKin> kinList = message.getNextOfKins();
    // merge vaccine, message header, and next of kin list with patient list
    MqePatient patient = message.getPatient();

    for (MqeVaccination v : vaccineList) {
