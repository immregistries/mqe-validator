...
public enum KnowNameList {
	...
	public boolean matches(NameType type, String first, String last, String middle) {
		...
		for (KnownName name : typeNames) {
			...
			if (!StringUtils.isEmpty(name.getNameLast()) && !StringUtils.isEmpty(last)
				&& name.getNameLast().equalsIgnoreCase(last)) {
				return true;
			}
			
			if (!StringUtils.isEmpty(name.getNameFirst()) && !StringUtils.isEmpty(first)
				&& name.getNameFirst().equalsIgnoreCase(first)) {
				return true;
			}
			
			if (!StringUtils.isEmpty(name.getNameMiddle()) && !StringUtils.isEmpty(middle)
