...
public class VaccinationVisDatesAreValid extends
	...
	protected ValidationRuleResult executeRule(DqaVaccination target,
		...
		if (publishedDateString == null) {
			...
			issues.add(Detection.VaccinationVisPresentedDateIsInvalid.build(vis.getPublishedDateString(), target));
		}
		...
		if (presentedDateString == null) {
			...
			issues.add(Detection.VaccinationVisPublishedDateIsInvalid.build(vis.getPresentedDateString(), target));
		} 
		...
		if (adminDate != null) {
			if (presentedDate != null) {
				if (datr.isAfterDate(presentedDate, adminDate)) {
					issues.add(Detection.VaccinationVisPresentedDateIsAfterAdminDate.build((presentedDateString), target));
				}
			}
			...
			if (presentedDate != null) {
				if (datr.isAfterDate(presentedDate, adminDate)) {
					...
					issues.add(Detection.VaccinationVisPresentedDateIsNotAdminDate.build((presentedDateString), target));
				}
			}
