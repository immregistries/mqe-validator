...
;
	  ...
	  private static final Map<VxuField, Map<DetectionType, Detection>> fieldIssueMaps = new HashMap<VxuField, Map<DetectionType, Detection>>();
	  ...
	  static {
		  for (Detection issue : Detection.values()) {
			  Map<DetectionType, Detection> map = fieldIssueMaps.get(issue.getTargetField());
			  if (map == null) {
				  map = new HashMap<DetectionType, Detection>();
				  fieldIssueMaps.put(issue.getTargetField(), map);
			  }
			  
			  map.put(issue.getDetectionType(), issue);
			  ...
			  displayTextMap.put(issue.getDisplayText() + issue.getDetectionType().getText(), issue);
	   ...
	   * @return
	   */
	  public static List<Detection> getIssuesFor(MessageObject object) {
  ...
  private Detection(MessageObject entity, DetectionType type,
  ...
  public DetectionType getIssueTypeEnum()
  {
    return detectionType;
  }
  ...
  public MessageObject getTargetObject()
  {
	...
	public ValidationDetection build(String value, MetaFieldInfoData meta) {
		ValidationDetection found = build(meta);
		found.setValueReceived(value);
		return found;
	}

  public ValidationDetection build(MetaFieldInfoData meta) {
	...
	public ValidationDetection build(Hl7Location loc) {
...
//
  ...
  public static Detection getPotentialIssueByDisplayText(String num, DetectionType missing) {
