...
public class CodeMasterManager
{
  ...
  public static CodeMaster getCodeMaster(CodedEntity codedEntity, CodeMaster codeMasterContext, Session session)
  {
    ...
    {
        List<CodeMaster> codeMasterList = new ArrayList<CodeMaster>();
    	if (codeMasterContext == null)
    	{
//        	Query query = session.createQuery("from CodeMaster where table.tableId = :param1 and context is null and codeValue = :param2");
//            query.setInteger("param1", codedEntity.getTableType().getTableId());
//            query.setParameter("param2", codedEntity.getCode());
//            codeMasterList = query.list();
    	}
    	else
    	{
//        	Query query = session.createQuery("from CodeMaster where table.tableId = :param1 and context = :param2 and codeValue = :param3");
//            query.setInteger("param1", codedEntity.getTableType().getTableId());
//            query.setParameter("param2", codeMasterContext);
//            query.setParameter("param3", codedEntity.getCode());
//            codeMasterList = query.list();
    	}
